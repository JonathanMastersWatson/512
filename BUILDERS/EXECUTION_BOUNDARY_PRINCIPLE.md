...
**Invariant 6 — Defined Fail Behavior** *(Directly computable)*
When the constraint kernel cannot complete evaluation — due to input ambiguity, system fault, timeout, or incomplete proposal object — the boundary routes to a defined fallback state. The governing rules apply even when evaluation cannot complete. The fallback state is pre-declared and domain-appropriate: fail-open (allow with gap record) or fail-closed (deny with gap record), depending on domain risk profile.

*Compilation note:* Directly computable. The fail behavior is a pre-declared configuration. "Fail-open" is appropriate for high-throughput, low-consequence domains. "Fail-closed" is appropriate for high-consequence domains where ungoverned execution is unacceptable.
...
| Invariant 6 — "fails open" | Changed to: "fails to pre-declared state (open or closed, domain-dependent)." | Strengthened — fail-open in a high-consequence domain is incorrect. The fail behavior must be domain-declared |
