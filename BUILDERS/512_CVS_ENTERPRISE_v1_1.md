...
Gate evaluation begins. Commit path requires gate authorisation signal. Infrastructure failure produces Evaluation-Unavailable DENY — commit path remains closed. Bypass paths generate gap records. Evidence chain is anchored continuously.
...
| Document | Purpose |
|---|---|
| `512_ARCHITECTURE_v3.5.md` | Full technical architecture — CTO/Board level |
| `CVS_ARCHITECTURE_v3.2.md` | CVS witness architecture — CTO/Board level |
| `512_IMPLEMENTATION_v3.4.md` | Build reference for Commit Gate — Engineer level |
...
| Field | Value |
|---|---|
| Document | `512_CVS_ENTERPRISE_v1_1.md` |
| Version | 1.1 |
| Date | June 2026 |
...
**Modifications:**
- §05 K6 description: "Fail-Open / Fail-closed behavior concentrates authority at the gate" replaced with the three-obligation decomposition — Evaluation-Unavailable DENY (commit boundary holds on gate failure), Transparent Denial (governing rule disclosed on DENY), Human Default (authority returns to human party on any adverse outcome).
- §07 Step 6: "Bypass paths generate gap records" supplemented with "Infrastructure failure produces Evaluation-Unavailable DENY — commit path remains closed."
- Reference Documents table: architecture and implementation references updated to v3.5 and v3.4 respectively.
