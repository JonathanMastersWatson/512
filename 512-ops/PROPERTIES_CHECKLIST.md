...
### Evaluation-Unavailable DENY Properties

- [ ] The gate produces Evaluation-Unavailable DENY when
      unavailable — commit path remains closed; execution
      does not proceed without admissibility established

- [ ] Gap records are generated by the CVS sidecar for all
      Evaluation-Unavailable DENY events

- [ ] Gap records identify the gap duration, reason, and
      gate output during the gap window (deny_evaluation_unavailable)
...
- [ ] For any CVS sidecar gap records (Evaluation-Unavailable DENY events): confirm
      the gap duration, reason, and gate_output_during_gap are recorded
